...
engine=build_engine('models/covid_model.onnx', shape = [1,200,250,3])
...
while(True):
   ...
   ret, frame = cap.read()
   curr_img_res=cv2.resize(frame,(250,200))
   curr_res_norm = np.expand_dims(curr_img_res ,axis=0)/255.0
   h_input_1, d_input_1, h_output, d_output, stream = allocate_buffers(engine, 1, trt.float32)

   out = do_inference(engine, curr_res_norm, h_input_1, d_input_1, h_output, d_output, stream, 1, 200, 250)
