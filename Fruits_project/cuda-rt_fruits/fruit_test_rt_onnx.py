...
engine=build_engine('models/fruits_model.onnx', shape = [1,250,250,3])
...
while(True):
   ...
   out = do_inference(engine, curr_res_norm, h_input_1, d_input_1, h_output, d_output, stream, 1, 250,250)
   #acc=acc+(time.time()-start_time)
   print("="*30)
   print(out)
   if out[0] < 0.5:
      print("Strawberry",out)
      cv2.putText(curr_img_res, 
                'Strawberry',
                (50, 50), 
