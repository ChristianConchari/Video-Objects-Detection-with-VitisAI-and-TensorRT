...
def make_tfrec(dataset_dir,tfrec_dir,img_shard):
  ...
  # unzip the blueberrys-vs-strawberrys archive that was downloaded from Kaggle
  ...
  for img in imageList:
    ...
    if class_name == 'strawberry':
        strawberryImages.append(img)
    else:
        blueberryImages.append(img)
  assert(len(strawberryImages)==len(blueberryImages)), 'Number of images in each class do not match'

  # define Train/test split as 80:20
  split = int(len(blueberryImages) * 0.2)

  testImages = blueberryImages[:split] + strawberryImages[:split]
  trainImages = blueberryImages[split:] + strawberryImages[split:]
...
def run_main():
  ...
  ap.add_argument('-s', '--img_shard',   type=int, default=1000,  help='Number of images per shard. Default is 1000') 
  args = ap.parse_args()
