...
def train(input_height,input_width,input_chan,tfrec_dir,batchsize,learnrate,epochs,chkpt_dir,tboard):
    ...
    chkpt_call = ModelCheckpoint(filepath=os.path.join(chkpt_dir,'numbers_model.h5'), 
...
def run_main():
    ...
    ap.add_argument('-ih','--input_height',type=int,   default=32,  help='Input image height in pixels.')
    ap.add_argument('-iw','--input_width', type=int,   default=32,  help='Input image width in pixels.')
    ...
    ap.add_argument('-b', '--batchsize',   type=int,   default=60,     help='Training batchsize. Must be an integer. Default is 50.')
    ap.add_argument('-e', '--epochs',      type=int,   default=15,    help='number of training epochs. Must be an integer. Default is 100.')
